...
import type { Withdrawal } from "@/types/types_withdrawals.ts";
import type { FinancialsStatus, StatusConfig } from "@/types/StatusConfig.ts";
import { approveWithdrawal, getAllWithdrawals, rejectWithdrawal } from "@/services/withdrawalsService.ts";
...
const WithdrawalsTable = () => {
    ...
    const handleApprove = async (withdrawalId: string) => {
        try {
            ...
            // Reload all withdrawals to get fresh data
            const data = await getAllWithdrawals();
            setWithdrawals(data.data);
            // Update selected withdrawal if modal is still open
            const updatedWithdrawal = data.data.find((w: Withdrawal) => w.id === withdrawalId);
    ...
    const stats = {
        ...
        totalFees: withdrawals
            .filter((w) => w.status !== "rejected")
            .reduce((sum, w) => sum + parseFloat(w.fee), 0),
    };
                                        ...
                                        <tr key={withdrawal.id} className="hover:bg-orange-50 transition">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div
                                                        className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center text-white font-bold">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        className="font-bold text-lg text-gray-800">{withdrawal.amount}</span>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-6">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[95vh] overflow-y-auto">
                        ...
                        <div className="p-4 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                ...
                                <div className="border-2 border-gray-200 rounded-xl p-4">
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        <h3 className="font-bold text-lg">{selectedWithdrawal.user.firstName} {selectedWithdrawal.user.lastName}</h3>
                                    </div>
                            ...
                            {/* Financial and Withdrawal Info */}
                            <div className="border-2 border-gray-200 rounded-xl p-6">
                                <div className="flex items-center gap-2 mb-4">
                                    <CreditCard className="w-5 h-5 text-green-600" />
                                    <h3 className="font-bold text-lg">Financial & Withdrawal Details</h3>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div className="p-3 bg-red-50 rounded-lg">
                                        ...
                                        <p className="font-bold text-lg text-red-600">-${selectedWithdrawal.fee}</p>
                                    </div>
                                    <div className="p-3 bg-green-50 rounded-lg border-2 border-green-200">
                                        <p className="text-sm text-gray-600 mb-1">Net Amount</p>
                                        <p className="font-bold text-lg text-green-600">${selectedWithdrawal.netAmount}</p>
                                    </div>
                                    ...
                                    <div className="p-3 bg-gray-50 rounded-lg">
                                        ...
                                        <div className="flex items-center gap-2 text-gray-700">
                                            ...
                                            <span
                                                className="text-sm font-medium">{formatDate(selectedWithdrawal.createdAt)}</span>
                                        </div>
                                    </div>
                                    {selectedWithdrawal.reviewedAt && (
                                        <div className="p-3 bg-gray-50 rounded-lg">
                                            ...
                                            <div className="flex items-center gap-2 text-gray-700">
                                                <Calendar className="w-4 h-4" />
                                                <span
                                                    className="text-sm font-medium">{formatDate(selectedWithdrawal.reviewedAt)}</span>
                                            </div>
                                        </div>
                                    )}
                                    {selectedWithdrawal.reviewedBy && (
                                        <div className="p-3 bg-gray-50 rounded-lg">
                                            ...
                                            <p className="font-semibold text-gray-800 capitalize">{selectedWithdrawal.reviewedBy}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
